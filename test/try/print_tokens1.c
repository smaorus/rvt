...
/***************************************************/
char unget_char(char ch, int *fp) //character_stream // ofer: used to be 'FILE *'
{ 
	char c;
  c=ungetc(ch,fp);
  if(c ==EOF)
...
static int is_token_end(int, char);
...
{ 
  ...
  int id=0;
  char ch,ch1[2];
  for (j=0;j<=80 /* shuvendu 2 */;j++)          /* initial the buffer   */
      { buffer[j]='\0';} 
   ch1[0]='\0';
   ch1[1]='\0';
   ch=get_char(tp);
   ...
   if(ch ==59)id=3; //ofer:change   /* prepare for comment */
   ch=get_char(tp);
...
{ int mytype;
 ...
 if(mytype==spec_symbol)print_spec_symbol(tok);
 if(mytype==identifier)
   {fprintf(stdout, "identifier,\"%s\".\n",tok);
   }
 if(mytype==num_constant)
   {fprintf(stdout, "numeric,%s.\n",tok);
   }
 if(mytype==str_constant)
   {fprintf(stdout, "string,%s.\n",tok);
   }
 if(mytype==char_constant)
   {tok=tok+1;
...
/******************************************/
static unget_error(character_stream fp)
//character_stream *fp; // originally had this redundant pointer
{
