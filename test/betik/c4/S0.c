...
typedef void (*FuncStackPtr)(struct stack*);
FuncStackPtr destroy_stackPtr = &destroy_stack;
void (*destroy_stackPtr2)(struct stack*);
...
void test1()
{
 ...
 do { seatest_assert_int_equal(TT_NUMBER, get_token(t), "<unknown>", 732); } while (0);
 do { seatest_assert_int_equal(TT_STRING, get_token(t), "<unknown>", 733); } while (0);
 do { seatest_assert_int_equal(TT_NUMBER, get_token(t), "<unknown>", 734); } while (0);
 release_tokenizer(t);
 free(t);
}
...
void tokenizer_test_fixture(void)
{
 do { seatest_test_fixture_start("./a.c"); } while (0);
 do { if(seatest_should_run("./a.c", "test1")) {seatest_setup(); test1(); seatest_teardown(); seatest_run_test(); }} while (0);
