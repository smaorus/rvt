...
// o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o
/// <summary> 
/// prints "<type> name", but removes one pointer dereference (by calling printBefore with 'remove_pointer == true').
/// </summary>

void Type::myprintType( std::ostream& out, Symbol *name) const // ofer
{
     printBase(out, 0);
	 out << " ";	    
     	 
    printBefore(out, name, 0, true);
...
// o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o+o
///<summary>
/// prints <type> name. We call it with remove_pointer=true, and then subsequent recursive calls are with remove_pointer=false. 
/// this has the effect of removing one pointer, e.g. *** v becomes **v;
///</summary>
void
PtrType::printBefore( std::ostream& out, Symbol *name, int level, bool remove_pointer ) const  //ofer
{
    if (subType)
    {
		if (subType->isPointer()) subType->printBefore(out,NULL,level, false);
        else subType->printBefore(out,NULL,level);
...
///<summary>
/// In case of a declaration of an argument without a name (headers decalared in libraries such as "double sqrt(double)") we add an artificial name because this header is later used as a header of a UF, where the names are necessary.
///</summary>
void add_name(Decl * arg) { // ofer. change tag: ADD_VAR_NAME_TO_DECLARATION
	static unsigned int counter = 0;
	Symbol* sym = new Symbol();
	std::stringstream s;

	s << counter++;
	std::string name = "rv_arg_" + s.str();			
	sym -> name  = name;
	sym -> entry = 
        gProject->Parse_TOS->transUnit->contxt.syms->Insert(mk_varfctdecl(name, arg));
	arg -> name = sym;		
}
...
FunctionType::addArgs(Decl *args)
{
    ...
    while (args != NULL)
    {
        args = args->next;
        arg->next = NULL;
		//assert(arg -> form); // ofer. 
		if (arg -> name == NULL && !RVCtool::is_basetype(arg -> form, BT_Void | BT_Ellipsis)) add_name(arg); // ofer. We do not add name in case of e.g. f(void). change tag: ADD_VAR_NAME_TO_DECLARATION
        addArg(arg);
        arg = args;
    }
}
...
Decl::dup0() const
{
    Decl *ret = new Decl();
    ret->storage    = storage;
    ret->form = form->dup(); // corrected by Dima
