...
// benny fixed (added if()) - FunctionDefs should get their old SymEntry :
			  if( !( declSym->entry && declSym->entry->IsFctDecl() &&
				 declSym->entry->scope && syms &&
				 declSym->entry->scope->level == syms->clevel ) )
