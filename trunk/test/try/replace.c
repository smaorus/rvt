// 1. through cil exposes the problem that the parser does not recognize GNU's '__attribute__ (( ...))'. 
// 2. had to change old-style declaration "f(a) int a {...} " to "f(int a) {"
// 3. a change in most functions (e.g. addstr) reveals the following problem: it fails with --pointer-check, because there is a pointer 
// offset there, or an array access where the offset size / array index is a parameter of the function.  
// one of the checks is that offset is only allowed within the object, and for arrays there is the --bounds-check. 
// If one day automatic assumptions will be available, it will solve it.


int isalnum(char); //ofer: added declaration.
void abort(); // ofer
...
int
makesub(char* arg, int from, character delim, char *sub)
	/*char*	arg;
	int	from;
	character	delim;
	char*	sub;*/
...
void subline();
 ...
 bool
locate(character c, char *pat, int offset)
	/*character	c;
	char *	pat;
	int	offset;*/
{
